...
class MCTS_Reward:
    ...
    class Node:
        def __init__(self, reward, parent, env, action, done):
    ...
    def __init__(self, env):
        ...
        self.env = env
        self.root = self.Node(reward=0, parent=None, env=env, action=-1, done=False)
            ...
            if current.n == 0:
                ...
                if not current.done:
                    ...
                    for action in range(self.env.action_space.n):
                        ...
                        node = self.Node(reward=reward, parent=current, env=env_current, action=action, done=done)
    ...
    def backpropagate(self, current, G):
